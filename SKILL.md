...
2. Stall re-assessment produced hard evidence of a physical floor — cite the evidence in `ITERATIONS.md`. On Tenstorrent the floors are:
   ...
   - **Matrix-engine FLOPS** for compute-bound kernels: achieved TFLOPS (2·M·N·K / time) near peak **for the dtype in use** — the peak is data-format/fidelity-dependent (Wormhole ≈ 50 TFLOPS bf16 / ≈ 190 bfp4; Blackhole ≈ 332 bf16 / 664 bfp8). Compare against the peak for the dtype you actually run, not the headline aggregate. Per-op utilization = ideal_cycles / actual_cycles.
